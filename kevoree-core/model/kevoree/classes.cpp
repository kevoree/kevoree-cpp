...
if(___mutatorType ==ADD){
addmetaData((Value*)AnyCast<KMFContainer*>(___value));
}else if(___mutatorType == REMOVE){
removemetaData((Value*)AnyCast<Value*>(___value));
}
...
if(relationName.compare("metaData")== 0){
return (KMFContainer*)findmetaDataByID(idP);
}
...
visitor->beginVisitRef("metaData","org.kevoree.Value");
for ( std::map<string,Value*>::iterator it = metaData.begin();  it != metaData.end(); ++it)
{
    Value * current =(Value*) it->second;
    
    internal_visit(visitor,current,recursive,containedReference,nonContainedReference,"metaData");
}
visitor->endVisitRef("metaData");
...
Value* Instance::findmetaDataByID(std::string id){
...
void Instance::addmetaData(Value *ptr)
{
    Value  *container = (Value *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be added in Instance because the key is not defined");
    }else
    {
        if(metaData.find(container->internalGetKey()) == metaData.end())
        {
            metaData[container->internalGetKey()]=ptr;
            	any ptr_any = container;
	RemoveFromContainerCommand  *cmd = new  RemoveFromContainerCommand(this,REMOVE,"metaData",ptr_any);
	container->setEContainer(this,cmd,"metaData");
...
void Instance::removemetaData(Value *ptr)
{
    Value *container = (Value*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be removed in Instance because the key is not defined");
    }
    else
    {
        metaData.erase( metaData.find(container->internalGetKey()));
        delete container;
        container->setEContainer(NULL,NULL,"");
    }
}
...
if(___refName.compare("deployUnits")==0){
if(___mutatorType ==ADD){
...
Value* TypeDefinition::findmetaDataByID(std::string id){
if(metaData.find(id) != metaData.end())
{
return metaData[id];
}else { return NULL; }
...
void TypeDefinition::addmetaData(Value *ptr)
{
    Value  *container = (Value *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be added in TypeDefinition because the key is not defined");
    }else
    {
        if(metaData.find(container->internalGetKey()) == metaData.end())
        {
...
void TypeDefinition::removemetaData(Value *ptr)
{
    Value *container = (Value*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be removed in TypeDefinition because the key is not defined");
    }
    else
    {
        metaData.erase( metaData.find(container->internalGetKey()));
...
Value* NetworkInfo::findvaluesByID(std::string id){
...
void NetworkInfo::addvalues(Value *ptr)
{
    Value  *container = (Value *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be added in NetworkInfo because the key is not defined");
    }else
    {
...
void NetworkInfo::removevalues(Value *ptr)
{
    Value *container = (Value*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be removed in NetworkInfo because the key is not defined");
    }
    else
    {
...
if(___mutatorType ==ADD){
addvalues((Value*)AnyCast<KMFContainer*>(___value));
}else if(___mutatorType == REMOVE){
removevalues((Value*)AnyCast<Value*>(___value));
}
}
...
visitor->beginVisitRef("values","org.kevoree.Value");
for ( std::map<string,Value*>::iterator it = values.begin();  it != values.end(); ++it)
{
    Value * current =(Value*) it->second;
...
Package* ContainerRoot::findpackagesByID(std::string id){
...
void ContainerRoot::addpackages(Package *ptr)
{
    Package  *container = (Package *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Package cannot be added in ContainerRoot because the key is not defined");
    }else
    {
        if(packages.find(container->internalGetKey()) == packages.end())
        {
            packages[container->internalGetKey()]=ptr;
            	any ptr_any = container;
	RemoveFromContainerCommand  *cmd = new  RemoveFromContainerCommand(this,REMOVE,"packages",ptr_any);
	container->setEContainer(this,cmd,"packages");
...
void ContainerRoot::removepackages(Package *ptr)
{
    Package *container = (Package*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Package cannot be removed in ContainerRoot because the key is not defined");
    }
    else
    {
        packages.erase( packages.find(container->internalGetKey()));
        delete container;
        container->setEContainer(NULL,NULL,"");
    }
}
...
} else if(___refName.compare("packages")==0){
if(___mutatorType ==ADD){
addpackages((Package*)AnyCast<KMFContainer*>(___value));
}else if(___mutatorType == REMOVE){
removepackages((Package*)AnyCast<Package*>(___value));
}
}
...
if(relationName.compare("packages")== 0){
return (KMFContainer*)findpackagesByID(idP);
}
...
visitor->beginVisitRef("packages","org.kevoree.Package");
for ( std::map<string,Package*>::iterator it = packages.begin();  it != packages.end(); ++it)
{
    Package * current =(Package*) it->second;
    
    internal_visit(visitor,current,recursive,containedReference,nonContainedReference,"packages");
}
visitor->endVisitRef("packages");
    }
    if(nonContainedReference)
    {
...
std::string Package::internalGetKey(){
return name;
}
Package* Package::findpackagesByID(std::string id){
if(packages.find(id) != packages.end())
{
return packages[id];
}else { return NULL; }
}
TypeDefinition* Package::findtypeDefinitionsByID(std::string id){
if(typeDefinitions.find(id) != typeDefinitions.end())
{
return typeDefinitions[id];
}else { return NULL; }
}
DeployUnit* Package::finddeployUnitsByID(std::string id){
if(deployUnits.find(id) != deployUnits.end())
{
return deployUnits[id];
}else { return NULL; }
...
void Package::addpackages(Package *ptr)
{
    Package  *container = (Package *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Package cannot be added in Package because the key is not defined");
    }else
    {
        if(packages.find(container->internalGetKey()) == packages.end())
        {
...
void Package::addtypeDefinitions(TypeDefinition *ptr)
{
    TypeDefinition  *container = (TypeDefinition *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The TypeDefinition cannot be added in Package because the key is not defined");
    }else
    {
...
void Package::adddeployUnits(DeployUnit *ptr)
{
    DeployUnit  *container = (DeployUnit *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The DeployUnit cannot be added in Package because the key is not defined");
    }else
    {
        if(deployUnits.find(container->internalGetKey()) == deployUnits.end())
        {
...
void Package::removepackages(Package *ptr)
{
    Package *container = (Package*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Package cannot be removed in Package because the key is not defined");
    }
    else
    {
        packages.erase( packages.find(container->internalGetKey()));
...
void Package::removetypeDefinitions(TypeDefinition *ptr)
{
    TypeDefinition *container = (TypeDefinition*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The TypeDefinition cannot be removed in Package because the key is not defined");
    }
    else
    {
...
void Package::removedeployUnits(DeployUnit *ptr)
{
    DeployUnit *container = (DeployUnit*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The DeployUnit cannot be removed in Package because the key is not defined");
    }
    else
    {
        deployUnits.erase( deployUnits.find(container->internalGetKey()));
...
string Package::metaClassName() {
return "Package";
}
void Package::reflexiveMutator(int ___mutatorType,string ___refName, any ___value, bool ___setOpposite,bool ___fireEvent){
if(___refName.compare("name")==0){
name= AnyCast<string>(___value);
}else {

if(___refName.compare("packages")==0){
if(___mutatorType ==ADD){
addpackages((Package*)AnyCast<KMFContainer*>(___value));
}else if(___mutatorType == REMOVE){
removepackages((Package*)AnyCast<Package*>(___value));
...
KMFContainer* Package::findByID(string relationName,string idP){
if(relationName.compare("packages")== 0){
return (KMFContainer*)findpackagesByID(idP);
}
...
void Package::visit(ModelVisitor *visitor,bool recursive,bool containedReference ,bool nonContainedReference)
{
...
void Package::visitAttributes(ModelAttributeVisitor *visitor){
visitor->visit(any(name),"name",this);
}
Package::Package(){
...
return hashcode+"/"+name+"/"+version;
}
Value* DeployUnit::findfiltersByID(std::string id){
...
void DeployUnit::addfilters(Value *ptr)
{
    Value  *container = (Value *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be added in DeployUnit because the key is not defined");
    }else
    {
...
void DeployUnit::removefilters(Value *ptr)
{
    Value *container = (Value*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be removed in DeployUnit because the key is not defined");
    }
    else
    {
...
if(___refName.compare("filters")==0){
if(___mutatorType ==ADD){
addfilters((Value*)AnyCast<KMFContainer*>(___value));
}else if(___mutatorType == REMOVE){
removefilters((Value*)AnyCast<Value*>(___value));
}
} else if(___refName.compare("requiredLibs")==0){
if(___mutatorType ==ADD){
...
visitor->beginVisitRef("filters","org.kevoree.Value");
for ( std::map<string,Value*>::iterator it = filters.begin();  it != filters.end(); ++it)
{
    Value * current =(Value*) it->second;
...
Value* Dictionary::findvaluesByID(std::string id){
if(values.find(id) != values.end())
{
return values[id];
}else { return NULL; }
...
void Dictionary::addvalues(Value *ptr)
{
    Value  *container = (Value *)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be added in Dictionary because the key is not defined");
    }else
    {
        if(values.find(container->internalGetKey()) == values.end())
        {
...
void Dictionary::removevalues(Value *ptr)
{
    Value *container = (Value*)ptr;
    if(container->internalGetKey().empty())
    {
        LOGGER_WRITE(Logger::WARNING,"The Value cannot be removed in Dictionary because the key is not defined");
    }
    else
    {
        values.erase( values.find(container->internalGetKey()));
...
std::string Value::internalGetKey(){
return name;
}
string Value::metaClassName() {
return "Value";
}
void Value::reflexiveMutator(int ___mutatorType,string ___refName, any ___value, bool ___setOpposite,bool ___fireEvent){
if(___refName.compare("name")==0){
name= AnyCast<string>(___value);
...
void Value::visit(ModelVisitor *visitor,bool recursive,bool containedReference ,bool nonContainedReference)
{
...
void Value::visitAttributes(ModelAttributeVisitor *visitor){
visitor->visit(any(name),"name",this);
